...
@synthesize openOffsetY = _openOffsetY;
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self)
    {
        ...
        [_headerView setArrowImage:[UIImage imageNamed:@"refresh_up.png"]];
        ...
        [_footerView setArrowImageDown:[UIImage imageNamed:@"refresh_down.png"]];
        _footerView.atTop = NO;
        [self addSubview:_footerView];
		
		self.enableHeaderPull = YES;
		self.enableFooterPull = YES;
        self.openOffsetY = kPROffsetY;
...
-(void)setOpenOffsetY:(CGFloat)openOffsetY{
        _openOffsetY = openOffsetY;
        self.headerView.openOffsetY =openOffsetY;
}
...
- (UIView *)emptyView{
    if (!_emptyView) {
        _emptyView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
        _emptyView.backgroundColor = [UIColor clearColor];
        
        UIImageView *emptyImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"empty_box"]];
        UILabel *emptyLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 150, 20)];
        ...
        emptyImageView.center = CGPointMake(_emptyView.center.x, _emptyView.center.y - 10);
        emptyLabel.center = CGPointMake(_emptyView.center.x, CGRectGetMaxY(emptyImageView.frame) + 15);
    }
    return _emptyView;
}
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (offset.y < -self.openOffsetY)     //header totally appeard
    {
         _headerView.state = kPRStatePulling;
    }
    else if (offset.y > -self.openOffsetY && offset.y < 0)    //header part appeared
    { 
        _headerView.state = kPRStateNormal;
        [_headerView updateRefreshDate:self.refreshDate format:dateFormatString];    
    }
    else if ( yMargin > self.openOffsetY)     //footer totally appeared
    {  
        if (_footerView.state != kPRStateHitTheEnd)
            _footerView.state = kPRStatePulling;
    }
    else if ( yMargin < self.openOffsetY && yMargin > 0)  //footer part appeared
    {
        if (_footerView.state != kPRStateHitTheEnd)
...
- (void)tableViewDidEndDragging:(UIScrollView *)scrollView
{
    ...
    if (_headerView.state == kPRStatePulling)
    {
        ...
        [UIView animateWithDuration:kPRAnimationDuration animations:^{
            self.contentInset = UIEdgeInsetsMake(self.openOffsetY, 0, 0, 0);
        }];
    ...
    else if (_footerView.state == kPRStatePulling)
    {
        ...
        [UIView animateWithDuration:kPRAnimationDuration animations:^{
            self.contentInset = UIEdgeInsetsMake(0, 0, self.openOffsetY, 0);
        }];
...
- (void)launchRefreshing
{
    ...
    [UIView animateWithDuration:kPRAnimationDuration*2 animations:^{
        self.contentOffset = CGPointMake(0, -self.openOffsetY-10.);
    } completion:^(BOOL bl){
